...
func goRenameGoModAction(ctx *cli.Context) error {
	...
	if old_mod_name, err = xgomod.GetName(); err != nil {
		return err
	}
	old_mod_name_bytes = []byte(old_mod_name)

	// 1.处理 go.mod
	if f_content, err := os.ReadFile(xgomod.FILE_NAME); err != nil {
		return err
	} else {
		f_content = bytes.Replace(f_content, old_mod_name_bytes, new_mod_name_bytes, -1)
		if err = os.WriteFile(xgomod.FILE_NAME, f_content, 0666); err != nil {
			return fmt.Errorf("save go.mod error:%W", err)
		}
	}
	// 2.处理 go 文件
	if rewrite_files, err = xos.DeepGlob(".", `*.go*`); err != nil {
		return err
	}
